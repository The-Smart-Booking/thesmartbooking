...
**Sprint atual:** Sprint 1 — Fim da Fatia 0 + frontend base (no board, sprint = campo `Iteration`; nenhum card tem iteração preenchida ainda)
...
| Próximos a puxar | 0.9 usuarios e memberships · 0.17 Postgres no CI · #54, #55 (frontend) |
| Fecha a Sprint 1 | 0.14 `RLS com FORCE, USING e WITH CHECK` + checklist da Fatia 0 no guia |
| Atenção | PRs #32 e #33 citam `Closes #9`/`#10`, que não existem mais, e têm conflito no `kanban.md` — por isso o CI nunca rodou nelas |
| Atenção | No board os 12 cards da Fatia 1 estão em `Ready`, mas #49–#53 e #57–#59 dependem da Fatia 0 |
...
- [ ] #54 — [T1] 1.7 Padronizar formato de resposta de erro da API · `ready`
- [ ] #55 — [T1] 1.8 Setup Vite + React + TypeScript + roteamento · `ready`
- [ ] #56 — [T1] 1.9 Cliente HTTP com tratamento centralizado de erro · `ready`
...
- [ ] #48 — [T1] 1.1 Config com TENANT_FIXO e PRESTADOR_FIXO (temporário, remover na T2) · `ready`
- [ ] #49 — [T1] 1.2 Camada de repositório: toda função recebe tenantID como 1º parâmetro · `ready`
- [ ] #50 — [T1] 1.3 Abrir transação por requisição com set_config('app.tenant_id', ...) · `ready` · **caminho crítico**
- [ ] #51 — [T1] 1.4 CRUD mínimo de clientes (criar e listar) · `ready`
- [ ] #52 — [T1] 1.5 POST /agendamentos traduzindo SQLSTATE 23P01 para HTTP 409 · `ready`
- [ ] #53 — [T1] 1.6 GET /agendamentos com filtro por período · `ready`
- [ ] #57 — [T1] 1.10 Tela de listagem de agendamentos · `ready`
- [ ] #58 — [T1] 1.11 Formulário de novo agendamento (data e hora digitadas, sem slots) · `ready`
- [ ] #59 — [T1] 1.12 Estados de carregamento e exibição de erro · `ready`
